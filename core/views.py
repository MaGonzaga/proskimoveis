...
def mostrar_duvidas(request):
    duvidas = Contato.objects.all()
    contexto = {
        "duvidas":duvidas
    }

    return render(request, 'sistema/duvidas.html', contexto)

def mostrar_delete(request,id):
    duvida = Contato.objects.get(id=id)
    #form = ContatoForm(request.POST or None, request.FILES or None, instance=duvidas)

    duvida.delete()
    return redirect('mostrar_duvidas')
...
def anuncio_site(request):
    anuncios = Anuncie.objects.all()
    return render(request, 'sistema/anuncio.html', {'anuncios': anuncios})
